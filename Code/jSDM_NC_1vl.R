...
dir.create(here("output", "one_LV"))
dir.create(here("output", "one_LV", "classification"))
dir.create(here("output", "plot"))
dir.create(here("output", "one_LV", "plot"))
dir.create(here("output", "RData"))
dir.create(here("output", "one_LV", "RData"))
...
data_site <- read.csv2(here("output", "data_site.csv"), sep = ",", dec = ".")
var_jSDM <- read.csv2(here("data_raw", "NCpippn", "var_site.csv"), sep = ",", dec = ".")
var_jSDM$X <- NULL
...
Sys.time()
save(jSDM_binom_pro, file = here("output","one_LV", "RData", "jSDM_binom_pro.RData"))
...
for (j in top_species[1]) {
  for (p in 1:np) {
    png(here("output", "one_LV", "plot", paste0("beta_jSDM_", p, ".png")))
    par(mfrow = c(1, 2))
...
for (j in top_species[1]) {
  png(here("output", "one_LV", "plot", "lambda_jSDM.png"))
  par(mfrow = c(2, 2))
  for (l in 1:n_latent) {
...
for (l in 1:n_latent) {
  png(here("output", "one_LV", "plot", paste0("W_", l, "_jSDM.png")))
  par(mfrow = c(2, 2))
  for (i in 1:2) {
...
png(here("output", "one_LV", "plot", "alpha_jSDM.png"))
plot(coda::as.mcmc(jSDM_binom_pro$mcmc.alpha[, 1:2]))
dev.off()

## V_alpha
png(here("output", "one_LV", "plot", "V_alpha_Deviance_jSDM.png"))
par(mfrow = c(2, 2))
...
## probit_theta
png(here("output", "one_LV", "plot", "theta_jSDM.png"))
par (mfrow = c(2, 1))
...
save(coord, species_to_plot, file = here("output", "one_LV", "RData", "obs_occ.RData"))
load(here("output", "one_LV", "RData", "obs_occ.RData"))
...
ggsave(here("output", "one_LV", "plot", "Observed_occurences.png"))

data_theta <- data.frame(coord[,c("longitude", "latitude")])
data_theta$theta <- jSDM_binom_pro$theta_latent[, species_to_plot]
save(data_theta, species_to_plot, file = here("output", "one_LV", "RData", "est_prob.RData"))
load(here("output", "one_LV", "RData", "est_prob.RData"))
...
ggplot(data = NC) +
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))
ggsave(here("output", "one_LV", "plot", "estimated_probabilities.png"))
...
save(species_richness, file = here("output", "one_LV", "RData", "species_richness.RData"))
load(here("output", "one_LV", "RData", "species_richness.RData"))
...
ggplot(data = NC) +
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))
ggsave(here("output", "one_LV", "plot", "species_richness_observed.png"))
...
ggplot(data = NC) +
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))
ggsave(here("output", "one_LV", "plot", "species_richness_estimated.png"))

png(here("output", "one_LV", "plot", "species_richness_observed_vs_estimated.png"))
par(mfrow = c(1,1))
...
system(glue('r.out.gdal --overwrite input=alpha_rst \\
             output={here("output", "one_LV", "alphas.tif")} type=Float32 \\
             createopt="compress=lzw,predictor=2"'))

# Representation
alpha_in <- st_crop(read_stars(here("output", "one_LV", "alphas.tif")), forest)
# scale to (0,1)
alpha_in[[1]] <- alpha_in[[1]] - mean(alpha_in[[1]], na.rm = TRUE)

write_stars(alpha_in, dsn = here("output", "one_LV", "alphas.tif"), options = c("COMPRESS=LZW", "PREDICTOR=2"))
plot(alpha_in, main = "Site effect alpha interpolated by RST")
alpha_xy <- rep(0, dim(coord)[1])
for (i in 1:dim(coord)[1]) 
{
  alpha_xy[i] <- as.numeric(system(glue('gdallocationinfo -l_srs {proj.t} -valonly {here("output", "one_LV", "alphas.tif")} \\
                                             -wgs84 {coord[i,"longitude"]} {coord[i,"latitude"]}'), intern = TRUE))
}
save(jSDM_binom_pro, alpha_xy, file = here("output", "one_LV", "RData", "alpha.RData"))
load(here("output", "one_LV", "RData", "alpha.RData"))

png(here("output", "one_LV", "plot", "alpha_estimated_vs_interpolated_by_RST"))
...
system(glue('r.out.gdal --overwrite input=W1_rst \\
             output={here("output", "one_LV", "lv_W1.tif")} type=Float32 \\
             createopt="compress=lzw,predictor=2"'))
# Representation
W1_in <- st_crop(read_stars(here("output", "one_LV", "lv_W1.tif")), forest)
# scale to (0,1)
W1_in[[1]] <- (W1_in[[1]] - mean(W1_in[[1]], na.rm = TRUE)) / sd(W1_in[[1]], na.rm = TRUE)
write_stars(W1_in, dsn = here("output", "one_LV", "lv_W1.tif"), options = c("COMPRESS=LZW", "PREDICTOR=2"))
plot(W1_in, main = "Latent variable W1 interpolated by RST")
W1_xy <- rep(0, dim(coord)[1])
for (i in 1:dim(coord)[1]) 
{
  W1_xy[i] <- as.numeric(system(glue('gdallocationinfo -l_srs {proj.t} -valonly {here("output", "one_LV", "lv_W1.tif")} \\
                                             -wgs84 {coord[i,"longitude"]} {coord[i,"latitude"]}'), intern = TRUE))
}
save(jSDM_binom_pro, W1_xy, file = here("output", "one_LV", "RData", "W1.RData"))
load(here("output", "one_LV", "RData", "W1.RData"))

png(here("output", "one_LV", "plot", "W1_estimated_vs_interpolated_by_RST"))
...
rst_alpha <- read_stars(here("output", "one_LV", "alphas.tif"))
rst_W1 <- read_stars(here("output", "one_LV", "lv_W1.tif"))
knn_alpha <- read_stars(here("output", "one_LV", "alpha_knn.tif"))
knn_W1 <- read_stars(here("output", "one_LV", "W1_knn.tif"))
...
params_species <- data.frame(species = species,
                             betas, lambda_1 = lambdas[, 1])
write.csv(params_species, file = here("output", "one_LV", "params_species.csv"), row.names = F)
params_species <- read.csv2(here("output", "one_LV", "params_species.csv"), sep = ",", dec = ".")

# Function to compute probit_theta at New Caledonia scale 
predfun <- function(scaled_clim_var, params_species, rst_alpha, rst_W1, species.range, Xbeta_aire){
...
for (n in 1:npart){
  ...
  write_stars(st_crop(merge(probit_theta), forest), options = c("COMPRESS=LZW", "PREDICTOR=2"),
              dsn = here("output", "one_LV", "theta", paste0("RST_probit_theta_", str_pad(n, width = 2, pad = "0"), ".tif")))
  ...
  write_stars(theta, options = c("COMPRESS=LZW", "PREDICTOR=2"), 
              dsn = here("output", "one_LV", "theta", paste0("RST_theta_", str_pad(n, width = 2, pad = "0"), ".tif")))
  write_stars(merge(st_crop(split(theta), forest)), options = c("COMPRESS=LZW", "PREDICTOR=2"), 
              dsn = here("output", "one_LV", "theta", paste0("RST_theta_forest_", str_pad(n, width = 2, pad = "0"), ".tif")))
  remove(theta)
}
...
theta <- read_stars(here("output", "one_LV", "theta", "RST_theta_forest_01.tif"))
latlong <- read.csv2(here("data_raw", "NCpippn", "coord_site.csv"), sep = ",")
latlong$X <- NULL
coord <- matrix(as.numeric(unlist(latlong)), nrow = nrow(latlong))[,2:3]
NC_PIPPN <- read.csv2(here("data_raw", "NCpippn", "data_clear.csv"), sep = ",")
...
save(theta, species_to_plot, file = here("output", "one_LV", "RData", "interpolated_current_specie.RData"))
load(here("output", "one_LV", "RData", "interpolated_current_specie.RData"))
...
ggplot() + 
  ...
  theme(plot.title = element_text(hjust = 0.5))
ggsave(here("output", "one_LV", "plot", "interpolated_presence_for_acropogon_NC.png"))
...
list_theta <- list.files(here("output", "one_LV", "theta"), pattern = "RST_theta_forest", full.names = TRUE)
...
terra::writeRaster(theta_sum, here("output", "one_LV", "theta_forest_sum.tif"), overwrite = TRUE)
theta_sum <- read_stars(here("output", "one_LV", "theta_forest_sum.tif"))
GT <- readOGR(here("data_raw", "Grande_Terre", "Grande_Terre.shp"))
...
ggplot() + 
  ...
  labs(fill = "Number of species") + 
  theme(plot.title = element_text(hjust = 0.5))
ggsave(here("output", "one_LV", "plot", "estimated_species_richness_forest.png"))  
...
theta_stars <- read_stars(here("output", "one_LV", "theta", "RST_theta_forest_01.tif"))[[1]]
ultramafic <- st_crop(read_stars(here("output", "environ_allNC.tif"))[,,,15], forest)[[1]]
theta <- terra::rast(here("output", "one_LV", "theta", "RST_theta_forest_01.tif"))
...
save(cell, file = here("output","one_LV", "RData", "cell.RData"))

# get position of each cell
npart <- 30
for (file in 2:npart) {
  increment <- 1
  theta_stars <- read_stars(here("output", "one_LV", "theta", paste0("RST_theta_forest_",str_pad(file, 2, pad = "0") , ".tif")))[[1]]
...
rm(theta_matrix_k)
save(theta_df, file = here("output", "one_LV", "RData", "theta_df.RData"))
...
# t SNE 
load(here("output", "one_LV", "RData", "theta_df.RData"))
...
png(here("output", "one_LV", "plot", "dendo_tSNE.png"))
...
save(pca_with_env, file = here("output", "one_LV", "RData", "pca_with_env.RData"))
load(here("output", "one_LV", "RData", "pca_with_env.RData"))

# keep axis while explained variance is more than 1% (8 in this case)
png(here("output", "one_LV", "plot", "PCA_explained_variances.png"))
fviz_eig(pca_with_env)
dev.off()
print(paste0("Explained variances by first eight axis : ", round(sum(fviz_eig(pca_with_env)$data[1:8,2])), "%"))

# CAH with PCA coord
dist_site <- dist(get_pca_ind(pca_with_env)$coord, method = "euclidian")
png(here("output", "one_LV", "plot", "dendo_ACP.png"))
...
save(tSNE_df, tSNE_site_group, file = here("output", "one_LV", "RData", "tSNE.RData"))
load(here("output", "one_LV", "RData", "tSNE.RData"))
...
ggsave(here("output", "one_LV", "plot", "tSNE_axis_1_2_group.png"))
...
ggplot(data = cbind(tSNE_df, tSNE_site_group),
  ...
  theme_bw() +
  theme(legend.position = "bottom", legend.title = element_blank()) 
ggsave(here("output", "one_LV", "plot", "tSNE_axis_2_3_group.png"))
...
save(pca_with_env, tSNE_site_group, file = here("output", "one_LV", "RData", "pca_tSNE.RData"))
load(here("output", "one_LV", "RData", "pca_tSNE.RData"))
# Axis 1 & 2 no var
fviz_pca_ind(pca_with_env, label = "none", fill = "orange", pointshape = 21, col.ind = "NA") + 
  theme(legend.text = element_text(size = 20),
        axis.title = element_text(size = 20),
        title = element_text(size = 20))
ggsave(here("output", "one_LV", "plot", "PCA_simple_axis_1_2.png"))
...
ggsave(here("output", "one_LV", "plot", "PCA_with_environ_sup_axis_1_2_tSNE.png"))
...
fviz_pca_biplot(pca_with_env, choix = "ind", label = c("quali", "quanti.sup"), repel = TRUE, col.quanti.sup = "black",
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))
ggsave(here("output", "one_LV", "plot", "PCA_with_environ_sup_axis_2_3_tSNE.png"))
...
save(col, file = here("output", "one_LV", "RData", "UM_col.RData"))
load(here("output", "one_LV", "RData", "UM_col.RData"))
...
fviz_pca_biplot(pca_with_env, choix = "ind", label = c("quali", "quanti.sup"), repel = TRUE, col.quanti.sup = "black",
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))
ggsave(here("output", "one_LV", "plot", "PCA_Axis_2-3_UM_NUM.png"))
...
save(col3d_UM, tSNE_df, file = here("output", "one_LV", "RData", "plot3d_UM.RData"))
##=====
## Change the coordinate scale for [10.255].
## for plot color for each pixel
##=====
save(pca_with_env, theta_df, tSNE_df, file = here("output", "one_LV", "RData", "RGB.RData"))
load(here("output", "one_LV", "RData", "RGB.RData"))
load(here("output", "one_LV", "RData", "cell.RData"))

# init stars object with CRS, extent, NA,...
rastRGB <- read_stars(here("output", "one_LV", "theta", "RST_theta_forest_01.tif"))[,,,1:3]
...
write_stars(rastRGB_tSNE, dsn = here("output", "one_LV", "RGB_forest_tSNE.tif"), 
            options = c("COMPRESS=LZW", "PREDICTOR=2"))
...
RGB_forest_tSNE <- terra::rast(here("output", "one_LV", "RGB_forest_tSNE.tif"))
colorize(RGB_forest_tSNE, value = 1:3, to = "col", stretch = "hist", filename = here("output", "one_LV", "RGB_forest_tSNE_one.tif"), overwrite = TRUE)
# RGB_forest_PCA <- read_stars(here("output", "RGB_forest_PCA.tif"))
RGB_forest_tSNE <- read_stars(here("output", "one_LV", "RGB_forest_tSNE.tif"))
...
R_G_B_plot
save_plot(filename = here("output", "one_LV", "plot", "R_G_B_forest_tSNE.png"), plot = R_G_B_plot, nrow = 3)

# get mean color of pixels for each group
load(here("output","one_LV", "RData", "cell.RData"))
load(here("output", "one_LV", "RData", "pca_tSNE.RData"))
RGB_forest_tSNE <- read_stars(here("output", "one_LV", "RGB_forest_tSNE_one.tif"))
...
RGB_forest_group <- read_stars(here("output", "one_LV", "RGB_forest_tSNE.tif"))
for (i in 1:dim(cell)[1]) {
  RGB_forest_group[[1]][cell[i,1], cell[i,2],] <- col_mean_group[pixel_col_hist[i, 4], ]
}
write_stars(RGB_forest_group, here("output", "one_LV", "RGB_forest_group.tif"))

RGB_forest_tSNE <- terra::rast(here("output", "one_LV", "RGB_forest_tSNE.tif"))
png(here("output", "one_LV", "plot", "RGB_forest_tSNE.png"))
par(mfrow = c(1, 1))
plotRGB(RGB_forest_tSNE, stretch = "hist")
title(main = "Predictives species community on forest area with tSNE",
      cex.main = 1.5, line = -2)
dev.off()

RGB_forest_group <- terra::rast(here("output", "one_LV", "RGB_forest_group.tif"))
...
ggsave(here("output", "one_LV", "plot", "RGB_forest_group_tSNE.png"))
...
for (h in 1:nb_class)
{
  ...
  writeLines(max_species_group[, 3 * h - 2], here("output", "one_LV", "classification", paste0("Group_", h, "_species_max.txt")))
  writeLines(min_species_group[, 3 * h - 2], here("output", "one_LV", "classification", paste0("Group_", h, "_species_min.txt")))
}
...
colnames(min_species_group) <- c(matrix(c(paste("Group", 1:nb_class), paste("Sd", 1:nb_class), 
                                          paste("Mean", 1:nb_class)), ncol = nb_class, byrow = TRUE))
write_csv2(data.frame(max_species_group), file = here("output", "one_LV", "classification", "max_species_group.csv"))
write_csv2(data.frame(min_species_group), file = here("output", "one_LV", "classification", "min_species_group.csv"))
...
alpha <- read_stars(here("output", "one_LV", "alphas.tif"))
latlong <- read.csv2(here("data_raw", "NCpippn", "coord_site.csv"), sep = ",")
latlong$X <- NULL
coord <- matrix(as.numeric(unlist(latlong)), nrow = nrow(latlong))[,2:3]
alpha_site <- rep(0, nrow = dim(coord)[1])
for (i in 1:dim(coord)[1]){
  alpha_site[i] <- as.numeric(system(glue('gdallocationinfo -l_srs {proj.t} -valonly {here("output", "one_LV", "alphas.tif")} \\
...
save(aire_site, alpha_site, file = here("output", "one_LV", "RData", "alpha_vs_area.RData"))
load(here("output", "one_LV", "RData", "alpha_vs_area.RData"))

png(here("output", "one_LV", "plot", "alpha_vs_area.png"))
...
save(df_sens, df_speci, file = here("output", "one_LV", "RData", "sens_speci.RData"))

load(here("output", "one_LV", "RData", "sens_speci.RData"))
NC <- ne_countries(scale = 10, returnclass = "sf")
...
ggplot(data = NC) +
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))
ggsave(here("output", "one_LV", "plot", "sensitivity_by_site.png"))
...
ggplot(data = NC) +
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))
ggsave(filename = here("output", "one_LV", "plot", "specificity_by_site.png"))
...
ggplot(data = NC) +
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))
ggsave(filename = here("output", "one_LV", "plot", "TSS_by_site.png"))
...
{
  group_site[i,] <- as.numeric(system(glue('gdallocationinfo -l_srs {proj.t} -valonly {here("output", "one_LV", "RGB_forest_group.tif")} \\
                                             -wgs84 {coord[i,1]} {coord[i,2]}'), intern = TRUE))
}
coord$group <- rowSums(round(group_site))
coord$group[is.nan(coord$group)] <- 0
coord$group[is.na(coord$group)] <- 0
coord$longitude <- as.numeric(coord$longitude)
coord$latitude <- as.numeric(coord$latitude)
for (i  in 1:6) {
...
ggplot() +
  ...
  scale_color_manual(values = c("#000000", color_group_hex[1:nb_class]), 
                     label = c("0" = "Hors forêt", "1" = "Groupe 1", "2" = "Groupe 2","3" = "Groupe 3",
                               "4" = "Groupe 4", "5" = "Groupe 5", "6" = "Groupe 6")) +
  scale_shape_manual(values = c(3, rep(16, nb_class)),
                     label = c("0" = "Hors forêt", "1" = "Groupe 1", "2" = "Groupe 2","3" = "Groupe 3",
                               "4" = "Groupe 4", "5" = "Groupe 5", "6" = "Groupe 6")) +
  coord_sf(xlim = c(163, 169 ), ylim = c(-22.84805679, -19.5), expand = FALSE) +
  theme_bw() +
